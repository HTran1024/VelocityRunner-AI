class Wall_Obstacle {
  ...
  int wallH = 30;  
  int wallW = 150;
  // int r will controls which wall is going to spawn, the left or right, through a random number generator.
  int r = floor(int(random(2)));
  
  // Moves the walls down at a certain speed.
  void move(float speed) {
    float actualSpeed = speed/7;
    posY += actualSpeed;
  }
  ...
  // Main function
  void obstacleGenerator(float speed) { 
    switch(r){
      case 0:
        leftWall();
        move(speed);
